...
int CustomPathsSM::GetMenuInternal()
{
	...
	else if (ret == ++Idx)
	{
		...
		if(strncasecmp(DeviceHandler::Instance()->GetFSName(Settings.NandEmuPath), "FAT", 3) != 0)
		{
	...
	else if (ret == ++Idx)
	{
		...
		if(strncasecmp(DeviceHandler::Instance()->GetFSName(Settings.NandEmuChanPath), "FAT", 3) != 0)
		{
